#!/bin/bash
# =============================================================================
#  Rathole Reverse Tunnel — v10
#  - Caddy removed completely
#  - WebSocket runs as direct reverse tunnel (no reverse proxy)
#  - Noise and TCP unchanged
#  - Added option to remove BBR / TCP Optimizer
# =============================================================================
...
banner() {
  clear
  echo -e "${C}=============================================================${N}"
  echo -e "${G}   Rathole Reverse Tunnel — v10 (No Caddy, BBR Removable)${N}"
...
install_core() {
  ...
  # Fallback to static musl if installed binary cannot run here
...
choose_proto() {
  ...
  echo "  3) WebSocket (Direct reverse tunnel, no Caddy)"
  echo ""
  read -rp "Choice [1]: " pc
  ...
  if [ "$PROTO" = "websocket" ]; then
    ...
    echo ""
    warn "WebSocket will run as a direct rathole reverse tunnel (no reverse proxy)."
    warn "Port 80 often works better for direct WebSocket than 443 without TLS."
...
EOF
  ...
  ok "Watchdog installed (checks every 5 seconds)."
...
apply_tcp_optimizer() {
  ...
  # If user previously removed BBR, remove the override first
  rm -f /etc/sysctl.d/99-z-remove-bbr.conf
...
# ---------- Remove BBR / TCP Optimizer ----------
remove_bbr_optimizer() {
  banner
  info "Removing BBR / TCP Optimizer and switching back to a safer default (cubic)."

  read -rp "Remove BBR/TCP optimizer now? [Y/n]: " ans
  if [[ "${ans:-Y}" =~ ^[Nn]$ ]]; then
    return
  fi

  # Stop/disable optimizer qdisc service
  systemctl stop tcp-optimizer-qdisc.service >/dev/null 2>&1
  systemctl disable tcp-optimizer-qdisc.service >/dev/null 2>&1

  # Remove optimizer files
  ...
  # If rathole tuning still forces BBR/fq, replace those lines with cubic/pfifo_fast
  if [ -f /etc/sysctl.d/99-rathole-anti-drop.conf ]; then
    sed -i 's/^net.ipv4.tcp_congestion_control = bbr/net.ipv4.tcp_congestion_control = cubic/' /etc/sysctl.d/99-rathole-anti-drop.conf 2>/dev/null || true
    sed -i 's/^net.core.default_qdisc = fq/net.core.default_qdisc = pfifo_fast/' /etc/sysctl.d/99-rathole-anti-drop.conf 2>/dev/null || true
  fi

  # Persistent override to make sure BBR stays disabled even if old files remain
  cat > /etc/sysctl.d/99-z-remove-bbr.conf <<'EOF'
...
EOF
  ...
  # Replace fq qdisc on interfaces where possible
  if command -v tc >/dev/null 2>&1; then
    for dev_path in /sys/class/net/*; do
      dev="$(basename "$dev_path")"
      [ "$dev" = "lo" ] && continue
      ...
      tc qdisc replace dev "$dev" root pfifo_fast 2>/dev/null || tc qdisc del dev "$dev" root 2>/dev/null || true
    done
  fi

  systemctl daemon-reload

  ok "BBR/TCP Optimizer removed."
  info "Current congestion control: $(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null)"
  info "Current default qdisc: $(sysctl -n net.core.default_qdisc 2>/dev/null)"
...
setup_kharej() {
  ...
  if ss -tn state established 2>/dev/null | grep -q ":${TUNNEL_PORT} "; then
    ...
    warn "3) WebSocket blocked by DPI/network path; try another port or protocol"
  fi
...
show_status() {
  ...
  info "TCP congestion: $(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null) | Default qdisc: $(sysctl -n net.core.default_qdisc 2>/dev/null)"
...
uninstall_all() {
  ...
  # Remove TCP optimizer files too
  rm -f /etc/sysctl.d/99-tcp-optimizer.conf
  rm -f /etc/modules-load.d/bbr.conf
  rm -f /usr/local/bin/tcp-optimizer-qdisc.sh
  rm -f /etc/systemd/system/tcp-optimizer-qdisc.service
  ...
  ok "Tunnel and related optimizer files fully removed."
  sleep 2
}
...
main_menu() {
  while true; do
    ...
    echo " 7) Remove BBR / TCP Optimizer"
    echo " 0) Exit"
    echo ""
